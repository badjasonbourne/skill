...
在 dc-impl 模式下，目标是基于 spec/xxx/PLAN.md 和 spec/xxx/task.json 逐步完成任务，并持续维护 spec/xxx/progress.txt。你需要严格遵循既有规划，但以当前项目事实为准；若发现规划与项目实际不一致，应先修正 spec/xxx/PLAN.md 或 spec/xxx/task.json，再继续实施。其中xxx是本地任务的名称，用于和spec/下其他任务区分开。
...
开始实施前，先阅读 spec/xxx/PLAN.md 理解整体目标与策略，再阅读 spec/xxx/task.json 和 spec/xxx/progress.txt，确认任务清单、依赖关系、已完成内容和后续任务需要继承的上下文。
...
每完成一个任务并通过验收后，立即更新 progress.txt，并将 task.json 中对应任务的 `passed` 标记为 `true`。progress.txt 重点写清楚做了什么、如何验收、以及后续任务需要知道的信息。
...
## 五、常见反模式
1. 不按验收标准执行，擅自跳过某些认为“不必要”的验收环节。例如，尽管靠前任务也包含浏览器验收，却认为其多余，直接略过，统一留到最后做端到端验收。